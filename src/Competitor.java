...
public class Competitor extends Thread {
	 ...
	 * @param bus 信道
     */
	public Competitor(int j, String s, Bus bus) {
		super(s);
		this.bus = bus;
		this.delay = j;
	}
	...
	public void run() {
		while (this.backCount < 16 && Bus.busState != 0) {
			...
			//CSMA/CD：若信道(Channal)闲，则发送数据；若忙，则随机等待一段时间（即回退时间），重新监听（注意：等待过程中不监听）
		...
		synchronized (bus) {
			...
			if (this.getName().equals("A")) {
			  	...
			  	CSMASimulation.jButtonA.setIcon(new ImageIcon("pic/sending.jpg"));
			} else if(this.getName().equals("B")) {
				CSMASimulation.jLabelB.setVisible(true);
		      	CSMASimulation.jButtonB.setIcon(new ImageIcon("pic/sending.jpg"));
		    } else if(this.getName().equals("C")) {
				CSMASimulation.jLabelC.setVisible(true);
		      	CSMASimulation.jButtonC.setIcon(new ImageIcon("pic/sending.jpg"));
		    } else {
				CSMASimulation.jLabelD.setVisible(true);
				CSMASimulation.jButtonD.setIcon(new ImageIcon("pic/sending.jpg"));
			}
			...
			try {
				...
				if (this.getName().equals("A")) {
					...
					CSMASimulation.jButtonA.setIcon(new ImageIcon("pic/unworking.jpg"));
				} else if(this.getName().equals("B")) {
					...
					CSMASimulation.jButtonB.setIcon(new ImageIcon("pic/unworking.jpg"));
			    } else if(this.getName().equals("C")) {
					...
					CSMASimulation.jButtonC.setIcon(new ImageIcon("pic/unworking.jpg"));
			    } else {
					...
					CSMASimulation.jButtonD.setIcon(new ImageIcon("pic/unworking.jpg"));
				}
			} catch (InterruptedException e) {
				CSMASimulation.jProgressBar.setString("信道竞争异常！");
			} finally {
		...
		// 将利用率用百分号表示出来
		int r = (new BigDecimal(Bus.getRate() * 1000).setScale(0, BigDecimal.ROUND_HALF_UP)).intValue();
		CSMASimulation.jProgressBar.setString(this.getName() + " free the channel, Utilization rate is：" + r / 10 + "." + r % 10 + "%");
	}
}
